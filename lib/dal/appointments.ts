...
import type {
  AppointmentRow, PersonRow, ProviderRow, AppointmentTypeRow,
}                                from '@/types/schema';
import type {
  AppointmentDetail, AppointmentInsert, AppointmentUpdate,
}                                from '@/types/dal';
...
async function enrich(
  ...
  const providerIds    = [...new Set(rows.map(r => r.provider_id).filter(Boolean) as number[])];
  const apptTypeIds    = [...new Set(rows.map(r => r.appointment_type_id).filter(Boolean) as number[])];

  const [people, providers, apptTypes] = await Promise.all([
    client.from('people').select('*').in('id', personIds)
      .then(r => (r.data ?? []) as PersonRow[]),
    providerIds.length > 0
      ? client.from('providers').select('*').in('id', providerIds).then(r => (r.data ?? []) as ProviderRow[])
      : Promise.resolve([]),
    apptTypeIds.length > 0
      ? client.from('appointment_types').select('*').in('id', apptTypeIds).then(r => (r.data ?? []) as AppointmentTypeRow[])
      : Promise.resolve([]),
  ]);
  ...
  return rows.map(row => ({
    ...
    appointment_type: row.appointment_type_id ? (typeMap.get(row.appointment_type_id) ?? null)     : null,
  }));
}
...
export async function getAllAppointments(client: Client): Promise<{
  ...
  const [{ data: upcoming, error: ue }, { data: past, error: pe }] = await Promise.all([
...
export async function deleteAppointment(client: Client, id: number): Promise<void> {
