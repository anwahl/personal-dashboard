...
export async function getTrackingData(
  ...
  const dateById = new Map<number, string>(
    (entries as { id: number; entry_date: string }[]).map(e => [e.id, e.entry_date])
  );
  ...
  for (const row of (numeric ?? []) as { entry_id: number; trackable_id: number; metric_value: number }[]) {
...
export async function getBooleanData(
  ...
  const byEntry = new Map<number, Record<number, number>>();
  for (const row of (habitData ?? []) as Pick<HabitEntryRow, 'entry_id' | 'trackable_id'>[]) {
    if (!byEntry.has(row.entry_id)) byEntry.set(row.entry_id, {});
