...
import type {
  PersonRow, DiagnosisRow, InfoGroupRow, InfoFieldTypeRow, InfoFieldValueRow,
  ItemListRow, ItemListEntryRow, LogSchemaRow, LogSchemaFieldRow,
  LogSchemaFieldOptionRow, LogEntryRow, LogEntryValueRow,
...
export async function getPersonPageData(
  ...
  const [
    ...
    client.from('diagnoses').select('*').eq('person_id', pid).eq('is_active', true)
      .then(r => (r.data ?? []) as DiagnosisRow[]),

    client.from('person_info_group_links').select('info_group_id').eq('person_id', pid)
      .then(r => (r.data ?? []).map((x: { info_group_id: number }) => x.info_group_id)),

    client.from('person_item_list_links').select('list_id').eq('person_id', pid)
      .then(r => (r.data ?? []).map((x: { list_id: number }) => x.list_id)),

    client.from('person_log_links').select('log_id').eq('person_id', pid)
      .then(r => (r.data ?? []).map((x: { log_id: number }) => x.log_id)),

    client.from('person_checklist_links').select('checklist_id').eq('person_id', pid)
      .then(r => (r.data ?? []).map((x: { checklist_id: number }) => x.checklist_id)),
  ...
  if (infoGroupLinks.length > 0) {
    const [groups, allFields] = await Promise.all([
      client.from('info_groups').select('*')
        .in('id', infoGroupLinks).eq('is_active', true).order('sort_order')
        .then(r => (r.data ?? []) as InfoGroupRow[]),
      client.from('info_field_types').select('*')
        .in('group_id', infoGroupLinks).eq('is_active', true).order('sort_order')
    ...
    infoGroups = groups.map(g => ({
      ...
      fields: allFields
        ...
        .map(f => {
          ...
          return { ...f, value: val?.field_value ?? null, value_id: val?.id ?? null };
        }),
    }));
  }
  ...
  if (logLinks.length > 0) {
    const [schemas, fields, allOptions, entries] = await Promise.all([
      client.from('log_schemas').select('*')
        .in('id', logLinks).eq('is_active', true).order('sort_order')
        .then(r => (r.data ?? []) as LogSchemaRow[]),
      client.from('log_schema_fields').select('*')
        .in('log_id', logLinks).eq('is_active', true).order('sort_order')
        .then(r => (r.data ?? []) as LogSchemaFieldRow[]),
      client.from('log_schema_field_options').select('*')
    ...
    const entryValues = entryIds.length > 0
      ? await client.from('log_entry_values').select('*').in('log_entry_id', entryIds)
          .then(r => (r.data ?? []) as LogEntryValueRow[])
      : [];

    // Group entry values by entry id
    const valuesByEntry = entryValues.reduce<Record<number, LogEntryValueRow[]>>((acc, v) => {
    ...
    logs = schemas.map(s => {
      ...
      const logEntries: LogEntryWithValues[] = entries
        ...
        .map(e => ({
          ...e,
          values: (valuesByEntry[e.id] ?? []).reduce<Record<number, string>>((acc, v) => {
  ...
  if (checklistLinks.length > 0) {
    const [lists, items] = await Promise.all([
      client.from('checklists').select('*')
        .in('id', checklistLinks).eq('is_active', true).order('sort_order')
        .then(r => (r.data ?? []) as ChecklistRow[]),
      client.from('checklist_items').select('*')
        .in('checklist_id', checklistLinks).order('sort_order')
  ...
  return { person, diagnoses, infoGroups, itemLists, logs, checklists, prescriptions };
}
...
export async function deleteItemListEntry(client: Client, id: number): Promise<void> {
...
export async function deleteLogEntry(client: Client, id: number): Promise<void> {
