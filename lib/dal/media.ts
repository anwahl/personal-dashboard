...
async function enrich(
  ...
  const [types, statusEntries, genreEntries] = await Promise.all([
    ...
    client
      ...
      .then(
        (r) =>
          (r.data ?? []) as Pick<
            MediaStatusEntryRow,
            "media_entry_id" | "status_id" | "status_date"
          >[],
      ),
    ...
    client
      ...
      .in("media_entry_id", entryIds)
      .then(
        (r) => (r.data ?? []) as { media_entry_id: number; genre_id: number }[],
      ),
  ]);
  ...
  const latestByEntry = new Map<
    number,
    { status_id: number; status_date: string }
  >();
  for (const se of statusEntries) {
    if (!latestByEntry.has(se.media_entry_id)) {
      latestByEntry.set(se.media_entry_id, {
        status_id: se.status_id,
        status_date: se.status_date,
      });
    }
  }
  ...
  const statusIds = [
    ...new Set([...latestByEntry.values()].map((s) => s.status_id)),
  ];
  const statuses =
    statusIds.length > 0
      ? await client
...
export async function getMediaEntries(
  ...
  let q = client
    ...
    .order("sort_order")
    .order("title");
  if (options?.mediaTypeId != null)
    q = q.eq("media_type_id", options.mediaTypeId);
...
export async function getStatusHistory(
  ...
  const statusMap = new Map(
    ((statuses ?? []) as MediaStatusRow[]).map((s) => [s.id, s]),
  );
