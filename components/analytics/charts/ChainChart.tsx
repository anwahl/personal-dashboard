...
interface Props {
  ...
  data:     TrackingDataPoint[];   // combined data; boolean trackables have value=1
  fromDate: string;
  toDate:   string;
}
...
// Layout constants
const CIRCLE_R = 5;
const STEP     = 20;   // px between circle centres
const LABEL_W  = 100;  // left column: emoji + name
const ROW_H    = 32;
const DATE_H   = 22;
const STREAK_W = 56;
const PAD_R    = 8;
...
export function ChainChart({ chart, data, fromDate, toDate }: Props) {
  ...
  if (!seriesLinks.length) {
    return (
      <div className="chart-block">
        ...
        <p className="empty-state">No series configured. Add boolean metrics in Settings → Charts.</p>
      </div>
    );
  }
  ...
  // Dates with ANY logged data (proxy for "daily entry exists")
  ...
  // Compute current (from-today-backwards) streak per series
  ...
  // Date label visibility: first, every Sunday, today
    ...
    <div className="chart-block">
      ...
      <div className="chart-scroll">
        <svg width={svgW} height={svgH} style={{ display: 'block' }}>
          ...
          {/* Date labels + guide dots */}
          {dates.map((date, i) => {
            if (!showDateLabel(date, i)) return null;
            const isToday = date === today;
            const label   = isToday ? 'Today' : date.slice(5); // MM-DD
            return (
              <g key={`dl-${date}`}>
          ...
          {seriesLinks.map((link, rowIdx) => {
            ...
            return (
              <g key={`row-${link.trackable_id}`}>
                {/* Emoji label */}
                <text x={2} y={rowCY + 5} style={{ fontSize: 14 }}>
                  {link.trackable.emoji ?? '•'}
                </text>
                {/* Name label */}
                <text x={22} y={rowCY + 4}
                  style={{ fontSize: 9.5, fill: 'var(--text-muted)', fontFamily: 'inherit' }}>
                  {link.trackable.name}
                </text>

                {/* Connecting lines (drawn before circles so circles appear on top) */}
                {dates.map((date, i) => {
                  if (i === 0) return null;
