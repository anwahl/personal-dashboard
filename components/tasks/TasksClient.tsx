...
function isReminderSoon(t: TaskDetail): boolean {
  if (!t.reminder_at || t.status.is_terminal || isReminderOverdue(t))
    return false;
            ...
            <>
              <div className="reminder-section__row">
                ...
                <span className="reminder-section__unit">
                  {form.recurrence_frequency === "daily"
                    ? "day(s)"
...
function FullAddForm({
  ...
  useEffect(() => {
    if (!form.reminder_at && form.due_date) {
      set("reminder_at", form.due_date + (form.due_time ? "T" + form.due_time : "T12:00:00"));
    }
  }, [form.due_date]);
...
export function TasksClient({
  ...
  const doneStatusId =
    ...
    0;
  const todoStatusId =
    statuses.find((s) => s.status_name === "todo")?.id ??
    statuses.find((s) => !s.is_terminal)?.id ??
    0;
  const normalPriorityId =
    priorities.find((p) => p.priority_name === "normal")?.id ??
    priorities[0]?.id ??
    0;
        ...
        const reminderPayload = data.reminder_at
          ? {
              ...
              recurrence_interval: data.recurrence_interval
                ? Number.parseInt(data.recurrence_interval)
                : null,
    ...
    <div>
      {showFull ? (
        <FullAddForm
          ...
          normalPriorityId={normalPriorityId}
          onSave={fullAdd}
        ...
        <>
          ...
          >
            ...
            <Button variant="ghost" onClick={() => setShowFull(true)}>
              Full Add
            </Button>
          </div>
        </>
      ...
      {tab === "active" && (
        <>
          {localActive.length === 0 && (
            <p className="empty-state">Nothing active — all clear! 🎉</p>
          )}
          {localActive.map((t) => (
            <div key={t.id}>
              {editTask?.id === t.id ? (
